// https://adventofcode.com/2024/day/6
package main

import (
	"errors"
	"flag"
	"fmt"
	"log"
	"os"
	"regexp"
	"strings"
	"sync"
	"sync/atomic"
	"time"
)

// #####################  <UTILS> #####################

var isTest *bool = flag.Bool("test", false, "Read the test input")

func timeTrack(start time.Time, name string) {
	elapsed := time.Since(start)
	log.Printf("%s took %s", name, elapsed)
}

func getFileInput(inputFile string) (string, error) {
	rawData, err := os.ReadFile(inputFile)
	if err != nil {
		return "", err
	}

	return string(rawData), nil
}

// ##################### </UTILS> #####################

type Dir int

const (
	N Dir = iota
	E
	S
	W
)

const (
	STARTING_POINT = '^'
	OBSTACLE       = '#'
	FREE_CELL      = '.'
	VOID           = '\000'
)

type Grid struct {
	Data      string
	RowLength int
	ColLength int
}

func (g Grid) GetValueIfExists(pos Position) byte {
	if g.PositionExists(pos) {
		return g.GetValue(pos)
	}

	return byte(VOID)
}

func (g Grid) GetValue(pos Position) byte {
	return g.Data[pos.ToIndex(g.RowLength)]
}

func (g Grid) PositionExists(pos Position) bool {
	return pos.X >= 0 && pos.X < g.RowLength && pos.Y >= 0 && pos.Y < g.ColLength
}

func (g Grid) GetStartingPoint() Position {
	ix := strings.IndexRune(g.Data, STARTING_POINT)

	return createPositionFromIndex(ix, g.RowLength)
}

func (g Grid) replaceDataWithObstacle(pos Position) string {
	var builder strings.Builder
	builder.WriteString(g.Data[:pos.ToIndex(g.RowLength)])
	builder.WriteRune(OBSTACLE)
	builder.WriteString(g.Data[pos.ToIndex(g.RowLength)+1:])
	return builder.String()
}

func (g Grid) InsertObstacle(pos Position) Grid {
	return Grid{
		Data:      g.replaceDataWithObstacle(pos),
		RowLength: g.RowLength,
		ColLength: g.ColLength,
	}
}

func (g Grid) String() string {
	var builder strings.Builder
	for i, ch := range g.Data {
		if i > 0 && i%g.RowLength == 0 {
			builder.WriteRune('\n')
		}
		builder.WriteRune(ch)
	}

	return builder.String()
}

func createGridFromString(dataString string) Grid {
	rowLength := strings.Index(dataString, "\n")
	colLength := strings.Count(dataString, "\n") + 1

	re := regexp.MustCompile(`\s`)

	return Grid{
		Data:      re.ReplaceAllString(dataString, ""),
		RowLength: rowLength,
		ColLength: colLength,
	}
}

type Position struct {
	X int
	Y int
}

func (p Position) ToIndex(rowLength int) int {
	return (p.Y * rowLength) + p.X
}

func (p Position) GetNeighbour(dir Dir) Position {
	switch dir {
	case N:
		return Position{p.X, p.Y - 1}
	case E:
		return Position{p.X + 1, p.Y}
	case S:
		return Position{p.X, p.Y + 1}
	case W:
		return Position{p.X - 1, p.Y}
	default:
		panic(fmt.Sprintf("Invalid direction: %v", dir))
	}
}

func createPositionFromIndex(ix, rowLength int) Position {
	return Position{
		X: ix % rowLength,
		Y: ix / rowLength,
	}
}

type DirectedPosition struct {
	Pos Position
	Dir Dir
}

func performWalk(grid Grid) (result []Position, isLoop bool, err error) {
	current_pos := grid.GetStartingPoint()
	visited_pos := make(map[Position]bool)
	visited_pos_with_dir := make(map[DirectedPosition]bool)
	current_dir := N

main_loop:
	for {
		visited_pos[current_pos] = true
		dir_pos := DirectedPosition{current_pos, current_dir}
		if visited_pos_with_dir[dir_pos] {
			isLoop = true
			break main_loop
		} else {
			visited_pos_with_dir[dir_pos] = true
		}

		next_pos := current_pos.GetNeighbour(current_dir)

		next_cell_value := grid.GetValueIfExists(next_pos)
		switch next_cell_value {
		case byte(OBSTACLE):
			current_dir = (current_dir + 1) % 4
		case byte(VOID):
			break main_loop
		case byte(FREE_CELL), byte(STARTING_POINT):
			current_pos = next_pos
		default:
			return nil, false, errors.New(fmt.Sprintf("Unknown cell type: %v", string(next_cell_value)))
		}
	}

	for k := range visited_pos {
		result = append(result, k)
	}

	return
}

func obstacleProducesLoop(grid Grid, pos Position) (bool, error) {
	updated_grid := grid.InsertObstacle(pos)

	_, isLoop, err := performWalk(updated_grid)
	if err != nil {
		return false, err
	}

	return isLoop, nil
}

func partOne(dataString string) (int, error) {
	defer timeTrack(time.Now(), "part one")

	grid := createGridFromString(dataString)
	visited_pos, _, err := performWalk(grid)
	if err != nil {
		return 0, err
	}

	return len(visited_pos), nil
}

func partTwo(dataString string) (int32, error) {
	defer timeTrack(time.Now(), "part two")

	grid := createGridFromString(dataString)
	visited_pos, _, err := performWalk(grid)
	if err != nil {
		return 0, err
	}

	var result int32
	var wg sync.WaitGroup

	for _, pos := range visited_pos {
		wg.Add(1)
		go func() {
			defer wg.Done()

			isLoop, err := obstacleProducesLoop(grid, pos)
			if err != nil {
				panic(fmt.Sprint(err))
			}

			if isLoop {
				atomic.AddInt32(&result, 1)
			}
		}()
	}

	wg.Wait()

	return atomic.LoadInt32(&result), nil
}

func main() {
	flag.Parse()

	var inputFile string
	if *isTest {
		inputFile = "input_test.txt"
		fmt.Println("Running 2024/6 solution on test input.")
	} else {
		inputFile = "input.txt"
		fmt.Println("Running 2024/6 solution on full input.")
	}

	data, err := getFileInput(inputFile)
	if err != nil {
		fmt.Printf("Error reading file:\n> %v", err)
		return
	}

	partOneSolution, err := partOne(data)
	if err != nil {
		fmt.Printf("Error solving part one:\n> %v\n", err)
		return
	}
	fmt.Printf("Part 1: %v\n", partOneSolution)

	partTwoSolution, err := partTwo(data)
	if err != nil {
		fmt.Printf("Error solving part two:\n> %v\n", err)
		return
	}

	fmt.Printf("Part 2: %v\n", partTwoSolution)
}
