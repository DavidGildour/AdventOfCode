...
def part_one(crabs: list[int]) -> int:
    ...
    you! They seem to be preparing to blast a hole in the ocean floor; sensors indicate a massive underground cave system just beyond where they're aiming!
    ...
    """
    median = stats.median(crabs)
    return sum(map(lambda x: abs(x - median), crabs))
...
def part_two(crabs: list[int]) -> int:
    ...
    """
    avg = stats.mean(crabs)

    def get_fuel_consumption(x, _avg):
        dist = abs(x - _avg)
        return dist * (dist + 1) / 2

    floored_score = sum(map(partial(get_fuel_consumption, _avg=math.floor(avg)), crabs))
    ceiled_score = sum(map(partial(get_fuel_consumption, _avg=math.ceil(avg)), crabs))
...
if __name__ == '__main__':
    ...
    print(f"PART TWO: The total number of movement necessary is equal to {int(total_movement)}.")
