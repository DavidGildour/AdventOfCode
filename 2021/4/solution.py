...
def part_one(numbers: list[int], board_list: list[BingoBoard]) -> int:
    ...
    while numbers and winning_board is None:
    ...
    return winning_board.get_score() * last_drawn_number


def part_two(numbers: list[int], board_list: list[BingoBoard]) -> int:
    ...
    last_drawn_number = None
    losing_board = None
    while numbers and len(board_list) > 0:
        last_drawn_number = numbers.pop(0)
        won_indices = []
        ...
        if len(won_indices) > 1:
            board_list = [bd for i, bd in enumerate(board_list) if i not in won_indices]
        elif len(won_indices) == 1:
            losing_board = board_list.pop(won_indices.pop())
...
if __name__ == '__main__':
    ...
    for line in board_data + [""]:
        if not line:
            boards.append(BingoBoard(board_buffer))
            board_buffer = []
        else:
            board_buffer.append([int(x) for x in line.split()])

    winning_score = part_one(draw_numbers, boards)
    print(f"PART ONE: The winning board's score is {winning_score}.")
    for b in boards:
        b.reset()
    losing_score = part_two(draw_numbers, boards)
    print(f"PART TWO: The losing board's (the last to win) score is {losing_score}.")
