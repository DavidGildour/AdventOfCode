...
from simulator import OctopiSimulator


def part_one(light_data: list[str]) -> int:
    ...
    """
    simulator = OctopiSimulator(light_data)
    return sum(simulator.process_step() for _ in range(100))


def part_two(light_data: list[str]) -> int:
    ...
    """
    simulator = OctopiSimulator(light_data)
    total_octopi = len(list(simulator.iter_area))
    for i in count(start=1):
        if simulator.process_step() == total_octopi:
            return i
...
if __name__ == "__main__":
    ...
    total = part_one(raw_data)
    print(f"PART ONE: A total of {total} octopi flashed in 100 steps.")

    sync_step = part_two(raw_data)
    print(f"PART TWO: The first step in which all the octopi flash is {sync_step}.")
