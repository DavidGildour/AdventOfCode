...
def part_one(report: list[str]) -> int:
    """
    ...
    for col in zip(*report):
        (most, _), (least, _) = Counter(col).most_common()
        gamma += most
        epsilon += least
...
def part_two(report: list[str]) -> int:
    ...
    for i in range(len(report[0])):
        if len(oxygen_candidates) > 1:
            relevant_oxy = (record[i] for record in oxygen_candidates)
        ...
        if len(co2_candidates) > 1:
            ...
            (_, c1), (least, c2) = Counter(relevant_co2).most_common()
            if c1 == c2:
                least = "0"
            co2_candidates = list(filter(lambda num: num[i] == least, co2_candidates))

        if len(oxygen_candidates) + len(co2_candidates) == 2:
            break

    oxy = oxygen_candidates.pop()
...
if __name__ == "__main__":
    ...
    power_cons = part_one(diag_report)
    print(f"PART ONE: The power consumption is {power_cons}.")
    life_support = part_two(diag_report)
    print(f"PART TWO: The life support rating is {life_support}.")
