...
def part_one(vents: list[Vent]) -> int:
    ...
    """
    straight_vents = filter(lambda v: v.is_horizontal or v.is_vertical, vents)
    vent_span = chain.from_iterable(map(Vent.get_full_range, straight_vents))
    c = Counter(vent_span)
    return sum(v > 1 for v in c.values())


def part_two(vents: list[Vent]) -> int:
    ...
    """
    vent_span = chain.from_iterable(map(Vent.get_full_range, vents))
    c = Counter(vent_span)
    return sum(v > 1 for v in c.values())
