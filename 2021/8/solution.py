...
from utils import LENGTH_TO_DIGIT, get_wire_mapping, parse_number


def part_one(displays: list[Display]) -> int:
    """You barely reach the safety of the cave when the whale smashes into the cave mouth, collapsing it. Sensors
    indicate another exit to this cave at a much greater depth, so you have no choice but to press on.
    ...
    segments randomly. Worse, the wire/segment connections are mixed up separately for each four-digit display! (All
...
def part_two(displays: list[Display]) -> int:
    ...
    total = 0
    for display in displays:
        wire_mapping = get_wire_mapping(display.unique_signals)
        total += parse_number(display.output, wire_mapping)
