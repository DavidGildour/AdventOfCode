...
def parse_commands(lines: list[str]) -> list[Command]:
    ...
    for line in lines:
        if line.startswith("$"):
            match line.split():
                case ["$", CmdType.LS]:
                    current_cmd = Command(CmdType.LS, None)
                case ["$", CmdType.CD, path]:
                    current_cmd = Command(CmdType.CD, path)
            parsed_commands.append(current_cmd)
        else:
...
def build_tree(commands_used: list[Command]) -> Directory:
    top_dir = None
    current_dir = None
    for cmd in commands_used:
        if cmd.type_ == CmdType.CD:
            if current_dir is None:
                top_dir = Directory(cmd.argument, current_dir)
                current_dir = top_dir
            elif cmd.argument == "..":
                current_dir = current_dir.parent
            else:
                current_dir = current_dir.children[cmd.argument]
        elif cmd.type_ == CmdType.LS:
            current_dir.add_children(cmd.output)

    return top_dir


def get_sum_of_sizes_below(threshold: int, top_dir: Directory) -> int:
    size = 0
    for child in top_dir.children.values():
        if isinstance(child, Directory):
            if child.size <= threshold:
                size += child.size
            size += get_sum_of_sizes_below(threshold, child)
...
def find_the_smallest_suitable_dir(
    ...
    for child in top_dir.children.values():
        if isinstance(child, Directory):
            ...
            if current_min > child.size >= space_to_free_up:
                current_min = child.size

    if next_level:
        return min(
            current_min,
            *[
                find_the_smallest_suitable_dir(space_to_free_up, directory, current_min)
                for directory in next_level
            ],
        )
    return current_min


def part_one(top_dir: Directory) -> int:
    """LYou can hear birds chirping and raindrops hitting leaves as the expedition proceeds. Occasionally,
    ...
    Within the terminal output, lines that begin with $ are commands you executed, very much like some modern computers:
        ...
        depends on the argument: cd x moves in one level: it looks in the current directory for the directory named x
        and makes it the current directory. cd .. moves out one level: it finds the directory that contains the
        current directory, then makes that directory the current directory. cd / switches the current directory to
        the outermost directory, /. ls means list. It prints out all of the files and directories immediately
        contained by the current directory: 123 abc means that the current directory contains a file named abc with
        size 123. dir xyz means that the current directory contains a directory named xyz.
    ...
    The total sizes of the directories above can be found as follows:
        ...
        directories. The directory a has total size 94853 because it contains files f (size 29116), g (size 2557),
        and h.lst (size 62596), plus file i indirectly (a contains e which contains i). Directory d has total size
        24933642. As the outermost directory, / contains every file. Its total size is 48381165, the sum of the size
        of every file.
    ...
    directories?
    """
    return get_sum_of_sizes_below(100_000, top_dir)


def part_two(top_dir: Directory):
    ...
    size_left = TOTAL_SIZE - top_dir.size
    space_required = MINIMUM_FREE_SPACE_REQUIRED - size_left
    return find_the_smallest_suitable_dir(space_required, top_dir, top_dir.size)
...
if __name__ == "__main__":
    ...
    root_dir = build_tree(commands)

    first_answer = part_one(root_dir)
    print(f"PART ONE: The answer to part one is equal to {first_answer}.")
    second_answer = part_two(root_dir)
