...
from typing import Collection, TypeVar, Generator, Any
...
def triplet_iter(collection: Collection[T]) -> Generator[list[T], Any, None]:
...
def part_one(data: list[str]) -> int:
    ...
    CrZsJsPPZsGzwwsLwLmpwMDw
        ...
        appears in both compartments is lowercase p. The second rucksack's compartments contain jqHRNqRjqzjGDLGL and
    ...
    for rucksack in data:
        ...
        compartment1 = set(rucksack[: rucksack_size // 2])
        compartment2 = set(rucksack[rucksack_size // 2 :])
        doubled_item = next(iter(compartment1 & compartment2))

        priority_score += priority[doubled_item]
...
def part_two(data: list[str]) -> int:
    ...
    The problem is that someone forgot to put this year's updated authenticity sticker on the badges. All the
    ...
    for rucksack1, rucksack2, rucksack3 in triplet_iter(data):
        badge = next(iter(set(rucksack1) & set(rucksack2) & set(rucksack3)))
        priority_score += priority[badge]
