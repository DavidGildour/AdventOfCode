def get_set_from_string(string: str) -> set[int]:
...
def part_one(data: list[str]):
    ...
    been assigned the job of cleaning up sections of the camp. Every section has a unique ID number, and each Elf is
    ...
    return sum(
        (
            set1 & set2 in [set1, set2]
            for set1, set2 in map(
                lambda ranges: (
                    get_set_from_string(ranges[0]),
                    get_set_from_string(ranges[1]),
                ),
                (pair.split(",") for pair in data),
            )
        )
    )
