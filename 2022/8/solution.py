from forest import Tree, Forest


def get_visible_trees(trees) -> set[Tree]:
    result_set = set()
    for line in trees:
        current_peak = 0
        for i, tree in enumerate(line):
            if i == 0 or tree.height > current_peak:
                result_set.add(tree)
                current_peak = tree.height
...
def part_one(forest: Forest) -> int:
    ...
    to block the view. In this example, that only leaves the interior nine trees to consider:
        ...
        of height 5 are in the way.) The top-middle 5 is visible from the top and right. The top-right 1 is not
        visible from any direction; for it to be visible, there would need to only be trees of height 0 between it
        and an edge. The left-middle 5 is visible, but only from the right. The center 3 is not visible from any
        direction; for it to be visible, there would need to be only trees of at most height 2 between it and an
        edge. The right-middle 3 is visible from the right. In the bottom row, the middle 5 is visible, but the 3 and
        4 are not.
    ...
    s1 = get_visible_trees(forest.from_west())
    s2 = get_visible_trees(forest.from_north())
    s3 = get_visible_trees(forest.from_east())
    s4 = get_visible_trees(forest.from_south())
...
def part_two(forest: Forest) -> int:
    ...
    35390
        ...
        immediately; it can see only 1 tree (of height 5, right next to it). Looking right, its view is not blocked;
        it can see 2 trees. Looking down, its view is blocked eventually; it can see 2 trees (one of height 3,
    ...
    return max(forest.get_scenic_score(tree) for tree in forest.tree_dict.values())
...
if __name__ == "__main__":
    ...
    input_forest = Forest.from_strings(raw_data)

    first_answer = part_one(input_forest)
    print(f"PART ONE: The answer to part one is equal to {first_answer}.")
    second_answer = part_two(input_forest)
